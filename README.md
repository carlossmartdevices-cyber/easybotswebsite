...
A fully functional e-commerce web application built with Next.js 15, featuring user authentication, ePayco payment integration, and AI-powered payment notifications.
...
- **Secure Payments**: Integration with ePayco payment gateway (USD and COP)
...
- **Payments**: ePayco Payment Gateway
...
- ePayco account with API credentials
...
1. Create an ePayco account at [https://epayco.com](https://epayco.com)
2. Navigate to your dashboard
3. Go to Integration > API Keys section
4. Copy your Public Key to `EPAYCO_PUBLIC_KEY` in `.env.local`
5. Copy your Private Key to `EPAYCO_PRIVATE_KEY` in `.env.local`
6. Set `EPAYCO_TEST_MODE=true` for testing (false for production)
7. Configure confirmation URL in ePayco dashboard:
   - URL: `https://your-domain.com/api/webhooks/epayco`
   - Method: POST
...
5. **Payment**: Complete payment through ePayco
6. **Confirmation**: Receive email confirmation and product access

### Admin Flow

1. **Webhook Reception**: ePayco sends payment confirmation to `/api/webhooks/epayco`
...
Creates an ePayco payment link for a product purchase.
...
```json
{
  "paymentLink": "https://checkout.epayco.co/...",
...
Receives ePayco confirmation events for payment status updates.

**Content-Type:** `application/x-www-form-urlencoded`

**Key Parameters:**
- `x_signature`: HMAC-SHA256 signature for verification
- `x_respuesta`: Payment status (Aceptada, Rechazada, Pendiente)
...
- Verify ePayco public and private keys
- Check ePayco dashboard for API errors
- Ensure test mode is properly configured
- Review server logs for ePayco SDK errors

### Webhook Not Receiving Events
- Verify confirmation URL in ePayco dashboard
- Check that private key matches for signature verification
- Ensure endpoint is publicly accessible
- Review server logs for signature verification errors
- Confirm webhook sends form data (not JSON)
...
Built with ❤️ using Next.js, Firebase, and ePayco
#   e a s y b o t s  
